...
async function handleRemote(req, res, sessionId) {
  ...
  const fallbackCall = FALLBACK_CHAIN[providerName] ?? callOpenAI;
  ...
  if (!result.ok) {
    ...
    if (fallbackCall) {
      console.warn(`[proxy] falling back to OpenAI`);
      result = await withRetry(() => fallbackCall(req, res.locals), 'openai-fallback');
      usedProvider = 'openai';
    }
